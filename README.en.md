...
| `POST /analyze/stream` | `/api/v1` | Bearer | business | `{question, session_id?, model?}` (`AskBody` byte-for-byte identical to `/analyze`; identity only from Bearer claims) | **SSE `text/event-stream`** (④a, ADR-0028 decision ④ · execution model A compute-then-stream): reuses `/analyze`'s single `agent.analyze()` path (all SQL through the Guard; the lock is released on return), then **replays the already-computed steps** as an AG-UI **event word-list** stream (word-list borrowed, **not an AG-UI protocol-compatibility claim**, N2). Sequence: `RUN_STARTED → per-role STEP_STARTED/TOOL_CALL_RESULT/STEP_FINISHED → STATE_SNAPSHOT → RUN_FINISHED`; the terminal `STATE_SNAPSHOT` carries the very same full `TurnPayload` as `/analyze` (single source of truth — the streamed terminal state is byte-identical). A rejected/failed step → `RUN_ERROR` right after that step's `STEP_STARTED` (rejected SQL never leaves the service, no `TOOL_CALL_RESULT`, N3). No analysis intent → only `RUN_STARTED/RUN_FINISHED` (the frontend falls back to request/response for the full TurnPayload). 422 session-identity-conflict and 503 snapshot-binding-failure share `/analyze`'s rules |
...
> The 15 rows fold the 18 OpenAPI paths (dual `/health` = 1 row; the `values` /
> `reports` collection+drill pairs = 1 row each; `POST /api/v1/analyze` added
> since 0026, `POST /api/v1/analyze/stream` since ④a); the path set is pinned by
> `tests/test_api_contract_v2.py` (`EXPECTED_PATHS`, 18 literals). All governance
...
40. Analysis UI is one workbench tab, not a standalone product: multi-step analysis ships both via the HTTP API (`POST /api/v1/analyze` request/response; `POST /api/v1/analyze/stream` SSE replay, ④a/ADR-0028 decision ④) and through the frontend console workbench "analysis" mode (`AnalysisBlock` renders the `analysis` projection; the streamed terminal state is byte-identical to request/response). Boundaries stated honestly: the stream is **compute-then-stream** (replays after `agent.analyze()` finishes, **not** SQL streamed while executing); event names **borrow the AG-UI word list, not AG-UI protocol compatible** (no AG-UI client can connect directly); ④b (SDK integration) remains gated. Governance panels read Git files + artifacts only, not an analysis entry. Full text: [README.md §10 #40](README.md).
