...
2. **前置快照复核**：`data/snapshot.py --check` 不通过就拒绝生成。值域一旦派生自
   漂移数据，就把漂移固化成了"权威口径"，比没有值域更糟。
...
def measure_column(
    ...
    if 0 < distinct_count <= max_cardinality:
        ...
        pairs = sorted(((_as_text(value), int(count)) for value, count in rows), key=lambda p: (-p[1], p[0]))
...
def _load_existing_aliases(
    path: Path, values: list[tuple[str, int]]
) -> dict[str, str]:
    ...
    if dangling:
        raise RuntimeError(
            f"{path.name} 的人工别名指向已不存在的取值（快照已变，请复核后手工修正）："
            f"{dangling}"
        )
    return aliases
...
def build_profile(
    ...
    return {
        ...
        "aliases": _load_existing_aliases(
            values_dir / f"{model.name}.{field}.json", pairs
        ),
        "note": note,
    }
...
def main() -> int:
    ...
    payloads = generate(domains, args.max_cardinality, args.dry_run)
    if not payloads:  # 域内无注册维度（模型退化）——不是成功，是配置异常
        print("[error] 未生成任何值域：域内没有可寻址维度（dimension_synonyms 为空）", file=sys.stderr)
        return 1
