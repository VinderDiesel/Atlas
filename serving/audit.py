...
口径（诚实声明）
--------------------
- **每业务请求一行**（/plan /compile /ask 处理结果 + 429/422 业务拒绝），
  字段全集恒定：ts / claims(role,sub) / endpoint / session_id / kind /
  row_count / latency_ms / status——无值一律 null，契约测试锁定字段集。
...
- kind 取值按端点语义如实：/plan → plan|clarify；/compile → compiled|
  compile_error；/ask → 回合 kind（answer|clarify|blocked|error|handoff）
  或 conflict（会话身份冲突 422）；限流命中 → rate_limited。
- 本地文件非防篡改（无签名/权限加固），生产应外置（README KL #28 ③ 收窄
  同步声明）；uvicorn workers=1（进程内，与会话/限流同声明）。
