...
- 三角色（serving/auth.py ROLE_DIRECTORY + row_policy.yml）：
  - hq_admin（总部）：条件 1=1 → 全量（跨域共用，与策略名无关）
  - 金融 branch_manager（分支经理）：dim_broker.branch = 本人分支 → 仅本分支
  - 金融 compliance_auditor（合规审计）：dim_customer.tier <= max_tier → 仅低敏感客户档
  - 零售 region_manager（州经理）：dim_store.s_state = 本人州 → 仅本州门店
  - 零售 category_analyst（品类分析师）：本州 + dim_item.i_category IN 本人品类集
...
产出
----
- stdout：每域三角色 SQL 谓词 / 行数 / Top 行
- eval/reports/rls-verify-<git sha>.json：结构化证据（绑定 HEAD，双域分节）
...
def _assert_distinct(outcomes: dict[str, dict], domain: str) -> bool:
    ...
    if len(outcomes) >= 2 and len(distinct) < 2:
        print(f"[error] {domain} 三角色结果未产生差异——权限下推未生效？", file=sys.stderr)
        return False
    return True


def run_finance(budget, args) -> tuple[dict, bool]:
    """金融档（gold-146 载体）：hq_admin / branch_manager / compliance_auditor。"""
    ...
    ok = True if args.no_execute else _assert_distinct(outcomes, "finance")
    section = {
...
def main() -> int:
    ...
    ap.add_argument(
        "--domain", choices=("finance", "retail", "all"), default="all",
