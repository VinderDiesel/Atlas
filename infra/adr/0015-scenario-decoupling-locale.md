...
英文词典（`patterns_en_us.yml`，B3b）节集合与中文**不强行对齐**——形态层本身就不
同构，对齐只会造出空节：

| 与中文的差异 | 内容 | 为什么 |
|---|---|---|
| `time.threshold_gate.words` | 裸年份兜底的**封锁表**（9 个阈值词） | 英文允许无介词裸年，中文不允许——这是英文独有的护栏，归时间节 |
| `months` | 小写月份名 → 1..12 | 月份名是词而非数字，必须换算；与 `time.patterns` 的 month 交替串同源（见下） |
| `topn_dim` | "top 3 categories …" 的名词后置短语提维度 | 中文"前 N 名"只给数字，无需此节 |
| `followup.what` / `instead` | 两条 `pattern`（而非词表 + 一条） | 英文追问无"开头词表"形态，靠句法尾锚 |
| `time.patterns[]` 条目的 `ref` | 引用中文词典的同名形态 | 见下（权威源唯一） |
| `time.patterns[]` 条目的 `flags` | 声明式正则标志（封闭白名单） | 见下（不重组正则） |

`ref` 用来解决**中英共享形态**：ISO 日期/季度与语言无关，若两边各写一份就成了同
一事实的两个可写来源（改一处漏一处）。`ref` 只能指向上游（zh 词典）已声明的 kind，
与 `pattern` 互斥必居其一，加载期解析为**同一已编译对象**（不副本）。因此英文形态
词典不独立：`load_locale_patterns("en_us")` 会连带加载 zh 词典。

`flags` 只为全仓唯一需要大小写不敏感的形态（月份名，客户会写 `may 2014`）放行：
它是声明式编译参数而非正则重组，且**只加在英文校验器**（`_time_pattern_items` 的
`shared` 参数）——中文词典带 `flags`/`ref` 仍报错，不把未经验证的能力摊开。
...
`{词典 kind} == set(分发表)`，不等即导入失败——新形态必须"词典 + 分发表"
同时落，不允许单边（中文 `_TIME_DISPATCH`，英文 `_EN_TIME_DISPATCH`）。
...
对照表见 `docs/design/adr-0015-pattern-lexicon-zh.md`（B3a）与
`adr-0015-pattern-lexicon-en.md`（B3b）。
...
4. 英文词典依赖中文词典（`ref`）：改 ISO 形态会同时影响两语——这是故意的（口径
   同源），但也意味着 `patterns_en_us.yml` 不能单独拷走复用；
5. 月份名是全仓唯一"同一事实两处写"（正则交替串 + `months` 表键）：交替串不能由
   代码重组（否则破硬约束 (a)），故加载器校"表键 → 模式命中"、测试校"交替串 →
   表键"，两方向合计等价于完全一致；
6. 重新评估触发条件：若出现**同一措辞在 zh/en 需要不同口径**的真实场景（当前不
