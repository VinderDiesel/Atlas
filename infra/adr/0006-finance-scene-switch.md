...
1. **目标区域生态聚焦**：目标城市为苏州/上海，长三角金融数据平台生态密度高于零售；
   金融场景下 FIBO 行业标准与公开基准（TPC-DI/BIRD finance）可复现闭环的验证价值更完整
2. **FIBO 引入的前提**（见 ADR-0007）：FIBO 是金融业务本体，
   零售场景下无对应锚点，引入意义不大；切换到金融后 FIBO 才有用武之地
3. **范围收窄**：从"通用零售分析"收窄为"金融数据治理 + 语义层 + AI"，口径更聚焦
...
| 自造数据（按 FIBO 概念虚构） | 概念与数据完全可控 | 评测可信度弱于公开基准，违背"可复现"原则；自证难度高 |
...
| schema 规模小于 TPC-DS | 表数量少，"湖仓"广度有限 | 如实标注范围：异构源 ETL + 语义层治理 + 可复现评测闭环，不夸大表规模 |
| 金融术语歧义风险 | counterparty/client/customer 等词在金融行业语义不一 | 术语表强化；语义模型 ai_context.synonyms 收敛；歧义问句必须反问（黄金集含 ambiguous 用例） |
| 场景切换可解释性 | 评审可能追问"为什么放弃零售" | 理由已固化在本 ADR：目标区域生态 + FIBO 锚点 |
...
- **目标领域转向非金融方向** → 重新评估场景选择，保留本 ADR 作为决策对照
