...
| `retail/` | gold-001 ~ 050 | 零售分析（TPC-DS，历史对照，不再新增——转正裁定待数据装载批次） | `atlas_retail_analytics` | 已建（2 条） |
...
- 当前：64 条（零售历史 2 + 金融主场景 62，其中歧义 6 条：零售 gold-047，金融 gold-104/121/122/148/155）——50 条目标已达成后按评测需要扩张（2026-09-03 filter 能力批次、2026-09-04 派生指标补洞批次）；2026-09-05 起样本按域分目录存储（本 README「目录结构与编号约定」节）
